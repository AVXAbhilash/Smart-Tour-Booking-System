...
import { MapPin, Clock, Users, Star, CheckCircle, ArrowLeft } from 'lucide-react';
import { tourData } from '../data/tours'; // Import our central data!

const TourDetails = () => {
  // 1. Grab the 'id' parameter from the URL
  const { id } = useParams();

  // 2. Find the tour in our data array that matches this ID
  const tour = tourData.find((t) => t.id === id);

  // 3. Fallback UI just in case someone types an invalid ID in the URL
  if (!tour) {
    return (
  ...
  // 4. Render the page dynamically using the 'tour' object
  return (
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-xl dark:shadow-primary-900/10 border border-gray-100 dark:border-slate-800 sticky top-28 transition-colors">
            <div className="text-3xl font-black text-slate-900 dark:text-white mb-2">
              ${tour.price} <span className="text-base font-medium text-gray-500 dark:text-slate-500">/ person</span>
            </div>
